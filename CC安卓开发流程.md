...
在开发和使用这套方案时，**浏览器开发环境**和**安卓打包后的生产环境**有本质区别，一定要提前考虑！

### 1. 同源策略（CORS）
| 环境 | CORS 限制 | 说明 |
|------|-----------|------|
| 桌面浏览器 | 有 | 只能请求同源域名，跨域会被浏览器拦截 |
| 安卓 WebView | 无 | Capacitor 打包后 WebView 中不受同源限制，可以直接请求任意 HTTPS 地址 |

**处理方式**：代码中根据环境自动切换请求逻辑
- 浏览器（开发时）：走本地代理服务器转发（如 `/api/webdav?url=...`）
- 安卓（APK）：直接请求真实地址（坚果云 WebDAV/GitHub API 等）

### 2. 本地代理服务器
- **用途**：仅用于桌面浏览器开发阶段，解决 CORS 问题
- **位置**：项目根目录的 `proxy-server.js`
- **说明**：APK 里没有这个代理服务器，也不需要它！
...
|---------|-----------|---------|
| `localStorage` | ✅ 可用 | ✅ 可用 |
| 原生文件系统 | ❌ 需额外插件 | ✅ 需 Capacitor Filesystem 插件 |
| 第三方云同步 | 需代理/CORS | 直接请求即可 |

---

## 记住原则
1. 开发时：浏览器需要代理
2. 生产时：打包进 APK 后直接请求，不需要代理
3. 所有涉及外部请求的功能，务必从架构层面就区分两套处理方式！
